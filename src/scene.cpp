...
static void draw_faces(FrameBuffer &buffer, const Vec3 light_dir,
  ...
  for (std::size_t i = 0; i < faces.size(); i++) {
    ...
    float bright_a{std::clamp(f.normal.dot(light_dir), 0.0f, 1.0f)};
    float bright_b{bright_a};
    float bright_c{bright_a};

    if (mode == RenderMode::Gouraud) {
      bright_a = std::clamp(first.normal.dot(light_dir), 0.0f, 1.0f);
      bright_b = std::clamp(second.normal.dot(light_dir), 0.0f, 1.0f);
      bright_c = std::clamp(third.normal.dot(light_dir), 0.0f, 1.0f);
    }
